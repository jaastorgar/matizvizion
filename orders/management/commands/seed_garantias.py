...
Idempotente (update_or_create por codigo). Ejecutar tras migrar:
...
POLITICAS = [
    {
        'codigo': 'legal', 'nombre': 'Garantía Legal (Ley 19.496 - SERNAC)',
        'base': PoliticaGarantia.BASE_LEGAL, 'orden_prioridad': 1,
        'plazo_label': '6 meses', 'plazo_dias_min': 180, 'plazo_dias_max': 180,
        'descripcion': (
        ...
        'permite_devolucion': True, 'permite_cambio': True,
        'permite_reparacion': True, 'permite_rehacer': False,
    },
    {
        'codigo': 'tecnica_armazon', 'nombre': 'Garantía Técnica - Armazones',
        'base': PoliticaGarantia.BASE_FABRICANTE, 'orden_prioridad': 2,
        'plazo_label': '12 meses', 'plazo_dias_min': 365, 'plazo_dias_max': 365,
        'descripcion': (
            "Desprendimiento de soldaduras, fallas espontaneas en bisagras o "
            "decoloracion anomala por defecto del material del armazon."
        ),
        'exclusiones': EXCLUSIONES_GENERALES,
        'permite_devolucion': False, 'permite_cambio': True,
        'permite_reparacion': True, 'permite_rehacer': False,
    },
    {
        'codigo': 'tecnica_tratamiento', 'nombre': 'Garantía Técnica - Tratamientos',
        'base': PoliticaGarantia.BASE_FABRICANTE, 'orden_prioridad': 3,
        'plazo_label': '6 a 12 meses', 'plazo_dias_min': 180, 'plazo_dias_max': 365,
        'descripcion': (
    ...
    {
        'codigo': 'adaptacion', 'nombre': 'Garantía de Adaptación Óptica (Confort Visual)',
        'base': PoliticaGarantia.BASE_CONFORT, 'orden_prioridad': 4,
        'plazo_label': '30 a 60 días', 'plazo_dias_min': 30, 'plazo_dias_max': 60,
        'descripcion': (
        ...
        'permite_devolucion': False, 'permite_cambio': False,
        'permite_reparacion': True, 'permite_rehacer': True,
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for data in POLITICAS:
            ...
            extra = data.pop('solo_con_receta', False)
            _, created = PoliticaGarantia.objects.update_or_create(
                codigo=codigo,
                defaults={**data, 'solo_con_receta': extra, 'activa': True},
            )
            n += 1
            self.stdout.write(f"  {'creada' if created else 'actualizada'}: {codigo}")
            data['codigo'] = codigo  # restaura por si se reusa la lista
