...
# Estados que, para el cliente, representan una COMPRA real (no un intento/abandono)
...
class CarritoViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        return ItemCarrito.objects.filter(carrito=self.get_carrito()).select_related('producto')
...
class OrdenViewSet(mixins.CreateModelMixin, mixins.ListModelMixin, mixins.RetrieveModelMixin, viewsets.GenericViewSet):
    serializer_class = OrdenSerializer
    permission_classes = [IsAuthenticated]
    def get_queryset(self):
        qs = Orden.objects.filter(cliente=self.request.user).select_related('cliente').prefetch_related('items__producto', 'historial').order_by('-creado_en')
    ...
    @transaction.atomic
    def create(self, request, *args, **kwargs):
        carrito = Carrito.objects.filter(cliente=request.user).prefetch_related('items__producto').first()
        ...
        productos = {p.pk: p for p in Producto.objects.select_for_update().filter(pk__in=cantidades.keys())}
        ...
        total = sum((productos[pid].precio * c for pid, c in cantidades.items()), Decimal('0'))
        orden = Orden.objects.create(cliente=request.user, total=total, estado=Orden.Estado.PENDIENTE)
        HistorialEstado.objects.create(orden=orden, estado_anterior='', estado_nuevo=Orden.Estado.PENDIENTE, usuario=request.user)
        for pid, cant in cantidades.items():
            prod = productos[pid]
            ItemOrden.objects.create(orden=orden, producto=prod, precio_unitario=prod.precio, cantidad=cant)
            prod.stock = F('stock') - cant
            prod.save(update_fields=['stock'])
        carrito.items.all().delete()
        return Response(self.get_serializer(orden).data, status=status.HTTP_201_CREATED)

class OperacionOrdenViewSet(mixins.ListModelMixin, mixins.RetrieveModelMixin, viewsets.GenericViewSet):
    serializer_class = OrdenSerializer
    permission_classes = [IsVendedorUser]
    def get_queryset(self):
        return Orden.objects.filter(estado__in=[Orden.Estado.PAGADA, Orden.Estado.EN_PREPARACION, Orden.Estado.ENVIADA]).select_related('cliente').prefetch_related('items__producto', 'historial').order_by('creado_en')
    @action(detail=True, methods=['patch'], url_path='actualizar-entrega')
    def actualizar_entrega(self, request, pk=None):
        nuevo_estado = request.data.get('estado')
        if not nuevo_estado:
            return Response({'error': 'Debes enviar el campo "estado".'}, status=status.HTTP_400_BAD_REQUEST)
