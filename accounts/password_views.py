...
class PasswordResetConfirmView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            print(f"[reset] uid NO decodifica a un usuario valido. uid={uid!r} error={e!r}")
        ...
        # Diagnostico: si falla, lo ves en la terminal del runserver
        print(f"[reset] check_token={ok} | email={user.email} | uid={uid!r}")
        if not ok:
            return Response({'error': 'Enlace invalido o expirado.'}, status=status.HTTP_400_BAD_REQUEST)
