...
class Tecnologo(models.Model):
    """
    Tecnólogo médico que atiende exámenes visuales en una sucursal.
    """
    sucursal = models.ForeignKey(
        'core.Sucursal',
        on_delete=models.CASCADE,
        related_name='tecnologos',
        db_index=True
    )
...
class BloqueHorario(models.Model):
    """
    Bloque de atención disponible de un tecnólogo en una fecha y horario.
    """
    tecnologo = models.ForeignKey(
        Tecnologo,
        on_delete=models.CASCADE,
    ...
    class Meta:
        ...
        constraints = [
            # Evita dos bloques idénticos del mismo tecnólogo
            models.UniqueConstraint(
                fields=['tecnologo', 'fecha', 'hora_inicio'],
                name='bloque_unico_tecnologo_fecha_inicio'
            )
        ]
    ...
    def clean(self):
        ...
        # 1. Hora de inicio debe ser anterior a hora de fin
        if self.hora_inicio and self.hora_fin and self.hora_inicio >= self.hora_fin:
            raise ValidationError('La hora de inicio debe ser anterior a la hora de fin.')

        # 2. No permitir solapamiento con otro bloque del mismo tecnólogo y fecha
        if self.tecnologo_id and self.fecha and self.hora_inicio and self.hora_fin:
            solapados = BloqueHorario.objects.filter(
                tecnologo=self.tecnologo,
                fecha=self.fecha,
            ...
            if solapados.exists():
                raise ValidationError(
                    'Este bloque se solapa con otro ya existente del mismo tecnólogo.'
                )
...
class CitaMedica(models.Model):
    """
    Cita de examen visual reservada por un cliente sobre un bloque horario.
    """
    Estado = EstadoCita
    ...
    class Meta:
        ...
        constraints = [
            models.CheckConstraint(
                condition=models.Q(estado__in=EstadoCita.values),
    ...
    def cancelar(self):
        """
        Cancela la cita y libera el bloque horario para que pueda reutilizarse.
        No permite cancelar una cita ya completada.
        """
        ...
        if self.estado == EstadoCita.CANCELADA:
            return  # Idempotencia: ya estaba cancelada
