...
function mediaHtml(p) {
  return p.imagen_url
    ? '<div class="mv-feat-emoji"><img src="' + p.imagen_url + '" alt="' + esc(p.nombre) + '" loading="lazy" /></div>'
...
function loadFeatured() {
  ...
  api.get('/store/productos/').then(function (r) {
    ...
    var seenG = {};
    dest = dest.filter(function (p) { if (p.grupo) { if (seenG[p.grupo]) return false; seenG[p.grupo] = true; } return true; }).slice(0, 4);
    renderFeatured(dest);
  });
}
initReveal();
